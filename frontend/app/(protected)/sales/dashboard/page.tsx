...
function CommissionStatusBadge({ status }: { status: 'pending' | 'approved' | 'paid' }) {
  ...
  return (
    <span className={['inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap', config.cls].join(' ')}>
      {config.label}
    </span>
  )
}
        ...
        <div className="max-w-5xl mx-auto px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-3">
            ...
            <span className="text-white/70 text-xs font-medium">
              Referral code: <span className="text-[#F4C430] font-bold">{salesRepProfile.referral_code}</span>
            </span>
          </div>
        </div>
      </div>
          ...
          <section className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white dark:bg-[#1f2937] rounded-2xl border border-[#E6D8BD] dark:border-[#374151] p-5">
                <p className="text-xs font-semibold uppercase tracking-wide text-[#C59F2C] mb-1">Pending</p>
                <p className="text-2xl font-bold text-[#0D3B2A] dark:text-white">
                  GH₵ {parseFloat(commissionData?.summary.pending ?? '0').toFixed(2)}
                </p>
              </div>
              <div className="bg-white dark:bg-[#1f2937] rounded-2xl border border-[#E6D8BD] dark:border-[#374151] p-5">
                <p className="text-xs font-semibold uppercase tracking-wide text-[#2E7D32] mb-1">Approved</p>
                <p className="text-2xl font-bold text-[#0D3B2A] dark:text-white">
                  GH₵ {parseFloat(commissionData?.summary.approved ?? '0').toFixed(2)}
                </p>
              </div>
              <div className="bg-white dark:bg-[#1f2937] rounded-2xl border border-[#E6D8BD] dark:border-[#374151] p-5">
                <p className="text-xs font-semibold uppercase tracking-wide text-[#0D3B2A] dark:text-[#F4C430] mb-1">Paid</p>
                <p className="text-2xl font-bold text-[#0D3B2A] dark:text-white">
            ...
            {!commissionsLoading && !commissionsError && (commissionData?.commissions.length ?? 0) === 0 && (
              <div className="p-8 bg-white dark:bg-[#1f2937] rounded-2xl border border-[#E6D8BD] dark:border-[#374151] text-center">
                <p className="text-sm text-[#5B3E31] dark:text-[#9ca3af]">No commissions yet.</p>
              </div>
            )}

            {!commissionsLoading && !commissionsError && commissionData?.commissions.map((com) => (
              <div
                key={com.id}
