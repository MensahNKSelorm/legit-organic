...
export default function B2BApplyPage() {
  ...
  useEffect(() => { if (state === 'sent') confirmationRef.current?.focus() }, [state])
  ...
  if (state === 'sent') return <div className="grid min-h-screen place-items-center bg-[#F4EFE4] px-6 text-center text-[#173C2A] dark:bg-[#171B18] dark:text-white"><div className="max-w-xl"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#246629] dark:text-[#F4C430]">Application received · LO-B2B-{reference}</p><h1 ref={confirmationRef} tabIndex={-1} className="display-organic mt-5 text-5xl md:text-7xl">Now we review.</h1><p className="mt-6 leading-7 text-[#675E52] dark:text-[#AFC0B2]">We’ll verify the organisation and contact you by email.</p><Link href="/b2b" className="mt-8 inline-block border-b border-current pb-1 text-sm font-bold">Return to business</Link></div></div>
  ...
  return <div className="min-h-screen bg-[#F4EFE4] pb-24 pt-28 text-[#173C2A] dark:bg-[#171B18] dark:text-white md:pt-36"><form onSubmit={submit} className="page-container grid gap-12 xl:grid-cols-[20rem_minmax(0,1fr)]">
    <aside className="xl:sticky xl:top-32 xl:h-fit"><Link href="/b2b" className="text-sm font-bold text-[#246629] dark:text-[#F4C430]">← Business</Link><p className="mt-10 text-xs font-bold uppercase tracking-[.16em] text-[#246629] dark:text-[#F4C430]">Trade application</p><h1 className="display-organic mt-4 text-5xl leading-[.9] md:text-7xl">Tell us who we’re supplying.</h1><p className="mt-7 max-w-sm text-sm leading-6 text-[#675E52] dark:text-[#AFC0B2]">Have your registration document and delivery details ready.</p><div className="mt-8 border-l-2 border-[#F4C430] pl-4 text-xs leading-5 text-[#675E52] dark:text-[#AFC0B2]">Documents are private and available only to authorised reviewers.</div></aside>
    <div className="space-y-14">
      ...
      <FormSection eyebrow="Contact" title="An authorised contact">
        <Field label="Full name"><input required autoComplete="section-applicant name" maxLength={150} value={form.contact_person} onChange={set('contact_person')} className={input}/></Field><Field label="Position"><input required autoComplete="section-applicant organization-title" maxLength={120} value={form.contact_job_title} onChange={set('contact_job_title')} className={input}/></Field><Field label="Work email"><input required type="email" autoComplete="section-applicant email" value={form.business_email} onChange={set('business_email')} className={input}/></Field><Field label="Primary phone"><input required type="tel" autoComplete="section-applicant tel" value={form.business_phone} onChange={set('business_phone')} className={input} placeholder="+233 or 0…"/></Field><Field label="Alternative phone" optional><input type="tel" autoComplete="section-alternative tel" value={form.alternative_phone} onChange={set('alternative_phone')} className={input}/></Field>
      </FormSection>
      <FormSection eyebrow="Delivery" title="Where goods are received">
        <Field label="Region"><select required autoComplete="section-delivery address-level1" value={form.delivery_region} onChange={set('delivery_region')} className={input}><option value="">Select region</option>{REGIONS.map(region=><option key={region}>{region}</option>)}</select></Field><Field label="City or town"><input required autoComplete="section-delivery address-level2" value={form.delivery_city} onChange={set('delivery_city')} className={input}/></Field><Field label="District / municipality" optional><input value={form.delivery_district} onChange={set('delivery_district')} className={input}/></Field><Field label="Locality / neighbourhood"><input required autoComplete="section-delivery address-line2" value={form.delivery_locality} onChange={set('delivery_locality')} className={input}/></Field><Field label="Street and building" optional wide><input autoComplete="section-delivery street-address" value={form.delivery_street} onChange={set('delivery_street')} className={input}/></Field><Field label="GhanaPost GPS" hint="Use a landmark and directions if unavailable."><input value={form.ghana_post_gps} onChange={set('ghana_post_gps')} className={input} placeholder="GA-123-4567" pattern="[A-Za-z]{2}-[0-9]{3,4}-[0-9]{4}"/></Field><Field label="Nearest landmark" optional><input value={form.delivery_landmark} onChange={set('delivery_landmark')} className={input}/></Field><Field label="Directions" optional wide><textarea rows={3} value={form.delivery_directions} onChange={set('delivery_directions')} className={input}/></Field><Field label="Receiving contact"><input required autoComplete="section-receiving name" value={form.receiving_contact_name} onChange={set('receiving_contact_name')} className={input}/></Field><Field label="Receiving phone"><input required type="tel" autoComplete="section-receiving tel" value={form.receiving_contact_phone} onChange={set('receiving_contact_phone')} className={input}/></Field><Field label="Receiving hours"><input required value={form.receiving_hours} onChange={set('receiving_hours')} className={input} placeholder="Mon–Fri, 8:00–16:00"/></Field><Field label="Access or unloading notes" optional><textarea rows={2} value={form.access_restrictions} onChange={set('access_restrictions')} className={input}/></Field>
      </FormSection>
