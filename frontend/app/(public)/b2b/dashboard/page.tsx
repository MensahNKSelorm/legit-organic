      ...
      <div className="min-h-screen bg-[#F4EFE4] pb-24 pt-32 text-[#173C2A] dark:bg-[#171B18] dark:text-white">
        <div className="page-container max-w-3xl">
          ...
          <p className="mt-6 max-w-xl text-[#675E52] dark:text-[#AFC0B2]">
            Purchasing tools are unavailable while this account is being reviewed. Contact us if you need help with the application.
          </p>
      ...
      <div className="page-container">
        ...
        {dataError && (
          <div className="flex flex-col gap-3 border-b border-[#C9BEAA] py-4 text-sm sm:flex-row sm:items-center sm:justify-between dark:border-white/15">
            <p>{dataError}</p>
            <button type="button" onClick={loadDashboard} className="w-fit border-b border-current font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F4C430]">Retry</button>
          </div>
        )}
        ...
        <section className="mt-16 border-t border-[#C9BEAA] pt-8 dark:border-white/15">
          <div className="flex items-end justify-between border-b border-[#C9BEAA] pb-4 dark:border-white/15"><div><h2 className="text-2xl font-semibold tracking-[-.03em]">Orders and receipts</h2><p className="mt-1 text-sm text-[#675E52] dark:text-[#AFC0B2]">The latest purchases made through this account.</p></div><Link href="/profile" className="text-sm font-bold">Full history →</Link></div>
          {receiptError && <p role="alert" className="border-b border-[#C9BEAA] py-3 text-sm text-red-700 dark:border-white/15 dark:text-red-300">{receiptError}</p>}
          {dataLoading ? <p className="py-8 text-sm text-[#675E52] dark:text-[#AFC0B2]">Loading orders…</p> : orders.length ? (
            <div className="divide-y divide-[#D8CEBC] dark:divide-white/15">{orders.slice(0, 6).map((order) => (
              <article key={order.id} className="grid gap-3 py-5 sm:grid-cols-[1.1fr_.7fr_.6fr_auto] sm:items-center">
                <div><p className="font-mono text-sm font-semibold">{order.reference}</p><p className="text-xs text-[#756D61] dark:text-[#98A59B]">{new Date(order.created_at).toLocaleDateString('en-GH', { dateStyle: 'medium' })}</p></div>
                <p className="text-sm font-semibold">{ORDER_LABELS[order.status]}</p>
                <p className="text-sm font-semibold">GH₵{Number(order.total_amount).toFixed(2)}</p>
