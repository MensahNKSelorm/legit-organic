...
export const metadata: Metadata = pageMetadata({
  ...
  description:
    "The terms governing the use of Legit Organic Limited’s website, ordering and delivery services.",
...
const sections = [
  ...
  {
    title: "Product quality and produce information",
    bullets: [
  ...
  {
    title: "Recipe Builder and user content",
    bullets: [
    ...
    <>
      <header id="top" className="bg-[#0D3B2A] pb-14 pt-36 text-white md:pb-20">
        <div className="page-container grid gap-8 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          <div>
            <p className="editorial-label text-[#F4C430]">The agreement</p>
            <h1 className="display-organic mt-4 text-6xl md:text-7xl xl:text-8xl">
      ...
      <div className="min-h-screen bg-[#FAF7F0] dark:bg-[#111827]">
        <div className="terms-document mx-auto max-w-4xl px-6 py-12 md:py-16">
          <p className="max-w-2xl text-lg leading-8 text-[#4B453D] dark:text-gray-300">
            These terms cover accounts, purchases, deliveries, weekly orders and use of Legit
            Organic’s digital services.
          </p>
          {sections.map((section, index) => (
            <section
              key={section.title}
              className="mt-10 border-t border-[#D8D0C1] pt-9 dark:border-white/15"
            >
              <div className="grid gap-3 md:grid-cols-[3rem_1fr]">
                <span className="font-mono text-xs text-[#8A7F6E] dark:text-gray-500">
                  {String(index + 1).padStart(2, "0")}
                </span>
                <div>
                  <h2 className="text-2xl font-semibold text-[#0D3B2A] dark:text-white">
                    {section.title}
                  </h2>
                  {section.body?.map((paragraph) => (
                    <p key={paragraph} className="mt-4 leading-7 text-[#3F3A34] dark:text-gray-300">
                      {paragraph}
                    </p>
                  ))}
                  {section.link && (
                  ...
                  {section.bullets && (
                    <ul className="mt-4 list-disc space-y-2 pl-5 leading-7 text-[#3F3A34] marker:text-[#C89F18] dark:text-gray-300">
          ...
          <section className="mt-12 border-t-2 border-[#0D3B2A] pt-8 dark:border-white">
            <h2 className="text-2xl font-semibold text-[#0D3B2A] dark:text-white">
              Questions about these terms?
            </h2>
            <p className="mt-3 text-[#3F3A34] dark:text-gray-300">
              Contact{" "}
              <a
          ...
          <div className="mt-10 border-t border-[#D8D0C1] pt-6 text-center dark:border-white/15">
            <a
              href="#top"
              className="font-semibold text-[#2E7D32] hover:underline dark:text-[#F4C430]"
            >
              ↑ Back to top
            </a>
          </div>
