...
export default async function ProductsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F0] text-[#0D3B2A] dark:bg-[#171B18] dark:text-[#FEFCF7]">
      ...
      <section
        id="market-grid"
        className="page-container scroll-mt-24 py-16 md:py-24"
        aria-labelledby="market-grid-title"
      >
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            <h2 id="market-grid-title" className="display-organic text-5xl md:text-7xl">
              Browse the stalls
            </h2>
        ...
        {products.length === 0 ? (
          ...
          <MarketGrid products={products} preview={usingDemo} featureFirst={!activeCategory} />
        )}
      </section>
    </div>
  );
