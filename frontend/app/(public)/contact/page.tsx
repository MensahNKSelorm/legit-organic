  ...
  return (
    <div className="min-h-screen bg-[#FAF7F0] dark:bg-[#171B18]">
      <EditorialPageHeader
        ...
        title={
          <>
            Let&apos;s talk <em className="font-normal text-[#F4C430]">properly.</em>
          </>
        }
        description="Orders, farmer partnerships, press questions or a simple hello. Choose the right contact below."
      />
      ...
      <section className="page-container py-14 lg:py-20">
        <div className="grid grid-cols-1 gap-14 lg:grid-cols-[.85fr_1.15fr] lg:gap-20">
          ...
          <div>
            <p className="editorial-label mb-6 text-[#2E7D32] dark:text-[#9FC5A4]">
              Where to find us
            </p>
              ...
              {contactItems.map((item) => {
                const card = (
                  <div className="group grid grid-cols-[2rem_1fr] gap-4 border-b editorial-rule py-5 text-[#0D3B2A] transition-colors hover:text-[#2E7D32] dark:text-white">
                    ...
                    <div>
                      <p className="text-xs font-bold text-[#8A6A22] dark:text-[#D7B951]">
                        {item.label.toLowerCase()}
                      </p>
                      <p className="mt-1 text-base font-semibold leading-snug">{item.value}</p>
                    </div>
                  </div>
                );
