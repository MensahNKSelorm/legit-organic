...
async function mockAuthenticatedSession(page: Page, subscriptions: unknown[] = []) {
  await page.route(apiPattern, async (route) => {
    ...
    if (path === "/api/users/b2b/status/" || path === "/api/sales/me/")
      return json(route, { status: null });
    if (path === "/api/orders/cart/")
      return json(route, { id: 1, items: [], created_at: "", updated_at: "" });
...
test.describe("critical customer journeys", () => {
  ...
  test("customer browses, searches, adds to cart, and sees safe checkout failure retry", async ({
    page,
  }) => {
    await mockGuestSession(page);
    let orderCreates = 0;
    ...
    await page.route("http://localhost:8000/api/orders/LO-QA-001/checkout/", (route) => {
      ...
      if (checkoutAttempts === 1)
        return json(route, { detail: "Payment service is temporarily unavailable." }, 503);
      return json(route, {
        checkout_url: "http://localhost:3000/payment?reference=LO-QA-001",
        reference: "PAY-QA-001",
      });
    });
  ...
  test("renewal payment failure remains retryable without changing subscription state", async ({
    page,
  }) => {
    const renewal = {
      id: 77,
  ...
  test("B2B application submits required organisation and delivery evidence", async ({ page }) => {
    await mockGuestSession(page);
    await page.route("http://localhost:8000/api/users/b2b/apply/", (route) =>
      json(route, { id: 42 }, 201)
    );
