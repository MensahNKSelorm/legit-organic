  ...
  return (
    <div className="mt-8 border-t editorial-rule pt-5">
      <div>
        ...
        <button type="button" onClick={addIngredient} className="mt-4 border-b border-current pb-1 text-xs font-bold text-[#2E7D32] dark:text-[#F4C430]">+ Add ingredient</button>
        {error && <p className="mt-3 text-sm text-red-600">{error}</p>}
        {saved && <p className="mt-4 text-sm font-bold text-[#2E7D32] dark:text-[#9FC5A4]">Saved to My Recipes.</p>}
        <button type="button" onClick={save} disabled={!dirty || saving} className="mt-5 bg-[#F4C430] px-6 py-3 text-sm font-bold text-[#0D3B2A] disabled:cursor-not-allowed disabled:opacity-35">{saving ? 'Saving…' : !dirty ? 'Make a change to save' : isAuthenticated ? 'Save to My Recipes' : 'Log in to save'}</button>
