...
function productOptions(ingredient: RecipeIngredient) {
  const options = [ingredient.product, ...ingredient.matched_products].filter(
    (product): product is IngredientProduct => Boolean(product?.is_available)
  );
  return options.filter(
    (product, index) => options.findIndex((item) => item.id === product.id) === index
  );
}

export default function RecipeShopIngredients({
  ingredients,
}: {
  ...
  const [selected, setSelected] = useState<Record<number, number>>(() =>
    Object.fromEntries(
      ingredients
        .map((ingredient) => [ingredient.id, productOptions(ingredient)[0]?.id])
        .filter(([, id]) => id)
    )
  );
    ...
    () =>
      ingredients.map((ingredient) => {
        ...
        return {
          ...
          options,
          product:
            options.find((option) => option.id === selected[ingredient.id]) || options[0] || null,
        };
      }),
            ...
            <div>
              ...
              {options.length > 1 ? (
                ...
                <p className="mt-1 text-xs text-[#5B3E31]/75 dark:text-[#B8D4BD]">
                  {product
                    ? `${product.name} · GHS ${product.price} / ${product.unit}`
                    : "Not currently matched to an available Market item"}
                </p>
              )}
            </div>
            {product ? (
              <button
                onClick={() => add(product)}
                className="inline-flex w-fit items-center gap-2 border-b border-current pb-1 text-sm font-bold text-[#0D3B2A] dark:text-[#F4C430]"
              >
                {added.includes(product.id) ? <>✓ Added</> : "Add one pack"}
              </button>
            ) : (
              <span className="text-xs font-semibold text-[#5B3E31]/55 dark:text-white/45">
                Unavailable
              </span>
            )}
          </div>
