...
export default function RecipeSearch({
  ...
  const options = useMemo(() => {
    ...
    const matches = recipes
      .filter(
        (recipe) =>
          !earlier.some((title) => normaliseRecipeText(title) === normaliseRecipeText(recipe.title))
      )
      .filter(
        (recipe) =>
          !current || normaliseRecipeText(recipe.title).includes(normaliseRecipeText(current))
      )
      .slice(0, 6);
  ...
  return (
    <form
      action="/recipes"
      method="get"
      className="relative mb-12 border-y editorial-rule py-5 lg:mb-16"
    >
      <label
        htmlFor="recipe-search"
        className="block text-sm font-bold text-[#0D3B2A] dark:text-[#F4C430]"
      >
        Find a recipe or build a plate
      </label>
      <div className="mt-3 flex items-end gap-3">
        ...
        <button
          type="submit"
          className="shrink-0 bg-[#F4C430] px-6 py-3.5 text-sm font-bold text-[#0D3B2A] transition-colors hover:bg-[#0D3B2A] hover:text-white dark:hover:bg-white dark:hover:text-[#0D3B2A]"
        >
          Search
        </button>
      </div>
        ...
        <div className="absolute inset-x-0 top-[calc(100%-1.25rem)] z-30 border border-[#0D3B2A]/20 bg-[#FAF7F0] shadow-xl dark:border-white/15 dark:bg-[#202720]">
          {options.map((option) => (
            <Link
              key={option.href}
              href={option.href}
              className="flex items-center justify-between gap-5 border-b border-[#0D3B2A]/10 px-5 py-4 last:border-0 hover:bg-[#F4C430]/20 dark:border-white/10"
            >
              <span className="display-organic text-xl text-[#0D3B2A] dark:text-white">
                {option.label}
              </span>
      ...
      <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs font-semibold text-[#5B3E31] dark:text-[#B8D4BD]">
        <span>Try:</span>
        <Link href="/recipes?q=fufu" className="border-b border-current">
          fufu
        </Link>
        <Link
          href="/recipes/combined?q=fufu%20%2B%20light%20soup"
          className="border-b border-current"
        >
          fufu + light soup
        </Link>
        <Link
          href="/recipes/combined?q=plain%20rice%20%2B%20kontomire%20stew"
          className="border-b border-current"
        >
          rice + kontomire stew
        </Link>
