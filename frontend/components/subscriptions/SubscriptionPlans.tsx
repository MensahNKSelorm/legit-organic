...
const PREVIEW_PLANS: SubscriptionPlan[] = [
  {
    ...
    audience: "household",
    plan_type: "curated",
    short_description: "A practical weekly mix for one.",
    weekly_price: "145.00",
    household_size: 1,
    image: null,
    is_featured: false,
  ...
  },
  {
    ...
    audience: "household",
    plan_type: "curated",
    short_description: "Staples and fresh produce for a family.",
    weekly_price: "295.00",
    household_size: 4,
    image: null,
    is_featured: true,
  ...
  },
  {
    ...
    slug: "large-household",
    audience: "household",
    plan_type: "curated",
    short_description: "A fuller basket for busy homes.",
    weekly_price: "460.00",
    household_size: 6,
    image: null,
    is_featured: false,
...
function BasketSizeMark({ size = 1 }: { size?: number | null }) {
  ...
  const produce = layouts[count as keyof typeof layouts].map(
    ([x, y, kind, rotation, scale], index) => ({
      x,
      y,
      kind,
      rotation,
  ...
  return (
    <svg
      className="subscription-basket h-20 w-20 text-[#75917C] opacity-55 group-hover:opacity-90 [--basket-fill:#FFFDF8] dark:text-[#A4BBA8] dark:[--basket-fill:#202620]"
      viewBox="0 0 64 64"
      fill="none"
      aria-hidden
    >
      ...
      <g clipPath={`url(#plan-basket-produce-${count})`}>
        {produce.map((item, index) => (
          ...
          >
            <g
              className="subscription-produce"
              style={{ "--produce-index": index } as CSSProperties}
            >
              {item.kind === 0 && (
                <>
                  <circle cx="0" cy="1" r="7" fill={item.color} stroke="currentColor" />
                  <path
                    d="M-1-6c0-4 2-6 5-7M2-8c4-2 7-1 8 2-4 2-7 1-8-2Z"
                    fill="#6E9B70"
                    stroke="currentColor"
                  />
                </>
              ...
              {item.kind === 1 && (
                <>
                  <circle cx="0" cy="1" r="8" fill={item.color} stroke="currentColor" />
                  <path
                    d="M-6 2c2-7 5-9 7-2 2-7 6-5 6 2M-4 6c1-5 4-6 5-1 2-5 4-4 5 0"
                    stroke="#DCE8D8"
                    strokeWidth="1.4"
                  />
                </>
              ...
              {item.kind === 2 && (
                <>
                  <path d="M-4-7C5-5 7 1 2 9l-8-3 2-13Z" fill={item.color} stroke="currentColor" />
                  <path d="M-3-7c-2-4 0-7 3-9M-2-7c3-4 6-5 9-3" stroke="currentColor" />
                </>
              )}
              {item.kind === 3 && (
                <>
                  <path
                    d="M-10-2c5 9 14 11 20 2-3 13-16 16-22 4"
                    fill="#F4C430"
                    stroke="currentColor"
                    strokeWidth="2.4"
                    strokeLinecap="round"
                  />
                  <path d="M-8-1c4 6 11 8 16 2" stroke="#8AA65E" />
                </>
              )}
              {item.kind === 4 && (
                <>
                  <path
                    d="M-9 2c2-8 8-11 13-6 7-1 9 7 4 11-7 5-15 2-17-5Z"
                    fill={item.color}
                    stroke="currentColor"
                  />
                  <path d="M-5 3c4-2 7-2 11 1" stroke="#DCE8D8" />
                </>
              )}
            </g>
          </g>
      ...
      <path
        d="M10 31h44l-5 25H15l-5-25Z"
        fill="var(--basket-fill)"
        stroke="currentColor"
        strokeWidth="1.7"
      />
      <path
        d="M10 31H54M18 42H50M16 49H49"
        stroke="currentColor"
        strokeWidth="1.7"
        strokeLinecap="round"
      />
    ...
    <div className="subscription-plan-grid grid gap-px overflow-hidden border border-[#D8CEBC] bg-[#D8CEBC] md:grid-cols-3 dark:border-white/15 dark:bg-white/15">
      {plans.map((plan, index) => (
        <article
          ...
          className="subscription-plan-card group relative flex min-h-[25rem] flex-col overflow-hidden bg-[#FFFDF8] p-7 md:p-8 dark:bg-[#202620]"
        >
          <div className="mb-auto flex items-start justify-between gap-4">
            <span className="text-xs font-bold uppercase tracking-[.18em] text-[#53705A] dark:text-[#A8C4AE]">
              {plan.household_size
                ? `${plan.household_size} ${plan.household_size === 1 ? "person" : "people"}`
                : "Flexible"}
            </span>
            <BasketSizeMark size={plan.household_size} />
          </div>
          <div className="mt-16">
            <h3 className="font-sans text-3xl font-semibold tracking-[-.04em] text-[#173C2A] dark:text-white">
              {plan.name}
            </h3>
            <p className="mt-3 max-w-[17rem] text-sm leading-6 text-[#675E52] dark:text-[#BBC8BD]">
          ...
          <div className="mt-10 border-t border-[#D8CEBC] pt-6 dark:border-white/15">
            <p className="text-2xl font-semibold tracking-[-.03em] text-[#173C2A] dark:text-white">
              GH₵{Number(plan.weekly_price).toFixed(0)}{" "}
              <span className="text-sm font-normal text-[#675E52] dark:text-[#A8B5AA]">/ week</span>
            </p>
            <Link
              href={`/subscriptions/start?plan=${plan.slug}`}
              className="mt-6 inline-flex items-center gap-3 border-b border-[#173C2A] pb-1 text-sm font-bold text-[#173C2A] transition-[gap] hover:gap-5 dark:border-[#F4C430] dark:text-[#F4C430]"
            >
