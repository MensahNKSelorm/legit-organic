...
export default function ProductCard({
  ...
  return (
    <article
      className={`group flex h-full min-h-[430px] flex-col overflow-hidden border-b border-[#0D3B2A]/20 bg-transparent dark:border-white/15 sm:min-h-[500px] ${featured ? "md:grid md:grid-cols-[1.15fr_.85fr]" : ""}`}
    >
      {/* Image */}
      <Link
      ...
      >
        <Image
          ...
          sizes={
            featured
              ? "(max-width: 640px) 50vw, (max-width: 768px) 100vw, 55vw"
      ...
      {/* Content */}
      <div
        className={`flex flex-1 flex-col p-3 sm:p-6 ${featured ? "md:justify-center md:p-10" : ""}`}
      >
        <h3
          className={`product-name-sans w-full break-words font-bold leading-tight text-forest-green dark:text-[#faf7f0] ${featured ? "mb-2 line-clamp-2 min-h-[2.5rem] text-base sm:min-h-[3.5rem] sm:text-2xl md:min-h-0 md:text-5xl" : "mb-2 line-clamp-2 min-h-[2.5rem] text-base sm:min-h-[3.5rem] sm:text-2xl"}`}
        >
          <Link
            href={`/products/${product.slug}`}
            className="transition-colors hover:text-[#2E7D32] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F4C430] dark:hover:text-[#F4C430]"
          >
            {product.name}
          </Link>
        </h3>
        ...
        <div
          className={`mb-2 flex min-h-[2.25rem] items-start sm:mb-3 ${featured ? "md:min-h-[28px]" : "sm:min-h-[44px]"}`}
        >
        ...
        <p
          className={`line-clamp-2 min-h-[2.5rem] text-[11px] leading-[1.25rem] text-charcoal/70 dark:text-[#d1d5db] sm:line-clamp-3 sm:min-h-[4.125rem] sm:text-sm sm:leading-relaxed ${featured ? "sm:flex-1" : ""}`}
        >
        ...
        <div className="mt-auto flex flex-col gap-3 border-t border-[#E6D8BD] pt-3 dark:border-[#374151] sm:mt-4 sm:flex-row sm:items-end sm:justify-between sm:pt-4">
          <div className="flex flex-col">
            ...
            <span
              className={`mt-1 text-[10px] font-semibold ${product.is_available ? "text-[#2E7D32] dark:text-[#9FC5A4]" : "text-red-700 dark:text-red-300"}`}
            >
