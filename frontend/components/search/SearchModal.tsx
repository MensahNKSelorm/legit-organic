        ...
        <div className="grid min-h-[72vh] md:grid-cols-[12rem_1fr] lg:grid-cols-[15rem_1fr]">
          <aside className="border-b border-[#0D3B2A]/20 bg-[#0D3B2A] px-6 pb-6 pt-6 text-white md:border-b-0 md:border-r md:border-white/15 md:px-7 md:py-9">
            <div className="flex items-start justify-between md:block">
              <h2 id="search-dialog-title" className="display-organic text-3xl text-[#F4C430]">
                Browse.
              </h2>
              <button
                onClick={onClose}
                aria-label="Close search"
                className="flex h-10 w-10 shrink-0 items-center justify-center border border-white/30 text-white transition-colors hover:border-[#F4C430] hover:text-[#F4C430] md:hidden"
              >
                ×
              </button>
            </div>
              ...
              <div className="mt-6 border-t border-white/25 md:mt-10">
                {categories.map((cat) => (
                  <button
                    ...
                    className="flex w-full items-center justify-between border-b border-white/20 py-3 text-left text-sm text-white/75 transition-colors hover:text-[#F4C430]"
                  >
                    <span>{cat.name}</span>
            ...
            {!query.trim() && (
              <div>
                <div className="mb-5 flex items-end justify-between border-b border-[#0D3B2A]/20 pb-3 dark:border-white/15">
                  <p className="text-xs font-bold uppercase tracking-[.16em] text-[#5B3E31]/65 dark:text-[#B8D4BD]">
                    Suggested
                  </p>
                  <Link
                    href="/products"
                    onClick={onClose}
                    className="text-xs font-bold text-[#2E7D32] dark:text-[#F4C430]"
                  >
                    View all ↗
                  </Link>
                </div>
            ...
            {!isLoading &&
              ...
              searchResult.related.length === 0 && (
                <div className="border-y border-[#0D3B2A]/20 py-10 text-left dark:border-white/15">
                  <p className="display-organic mb-7 break-words text-4xl text-[#0D3B2A] dark:text-white">
                    Nothing found.
                  </p>
                  <Link
                    href="/products"
