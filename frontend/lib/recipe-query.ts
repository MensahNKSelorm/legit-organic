export const normaliseRecipeText = (value: string) => value.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim()
...
export function parseRecipeDraft(value: string, catalogue: string[]): { selected: string[]; fragment: string } {
  const explicitParts = value.split(/\s*(?:\+|,|\band\b)\s*/i)
  if (explicitParts.length > 1) {
    return {
      selected: explicitParts.slice(0, -1).map(part => canonicalTitle(part, catalogue) || part.trim()).filter(Boolean),
