...
class PromoCode(models.Model):
    ...
    code = models.CharField(max_length=50, unique=True, help_text='The promo code e.g. KWAME20')
    description = models.TextField(blank=True, help_text='Internal note about this code')
    ambassador_name = models.CharField(
        max_length=100, blank=True, help_text='Brand ambassador this code belongs to'
    )
    ambassador_email = models.CharField(max_length=255, blank=True)

    discount_type = models.CharField(
        max_length=20, choices=DISCOUNT_TYPES, default=DISCOUNT_PERCENTAGE
    )
    discount_value = models.DecimalField(
        max_digits=10, decimal_places=2, help_text='e.g. 20 for 20% or 10 for GH₵10 off'
    )
    minimum_order_amount = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, help_text='Minimum order value to use this code'
    )
