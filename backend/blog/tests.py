...
class WeeklyBlogCommandTests(TestCase):
    ...
    def test_pick_topic_prefers_unused_and_skips_recent(self):
        from blog.management.commands.generate_weekly_blog import (
            _pick_topic,
            _key,
...
class GenerationTests(TestCase):
    ...
    def test_generate_post_sanitises_and_appends_sources(self, m_call):
        ...
        d = generate_post(
            'topic',
            [{'title': 'S1', 'snippet': 'x', 'url': 'http://example.com/a', 'source': 'Web'}],
        )
    ...
    def test_empty_draft_rejected(self, m_call):
        ...
        with self.assertRaises(ValueError):
            generate_post(
                'topic', [{'title': 'S', 'snippet': 'x', 'url': 'http://e/1', 'source': 'Web'}]
            )
