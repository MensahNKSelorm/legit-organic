...
def security_setup(request):
    ...
    if device is None:
        device = TOTPDevice.objects.create(
            user=request.user,
            name='Legit Organic authenticator',
            confirmed=False,
        )
...
def _owner_reauthenticated(request):
    password_ok = request.user.check_password(request.POST.get('current_password') or '')
    verified, recovery_used = verify_staff_code(request.user, request.POST.get('otp_token') or '')
...
def exceptional_delete(request, app_label, model_name, object_id):
    ...
    if request.method == 'POST':
        ...
        if not reason:
            ...
            return _secure(
                render(
                    request,
                    'security/destructive_action.html',
                    {
                        'complete': True,
                        'action_name': 'Permanent deletion',
    ...
    return _secure(
        render(
            ...
            {
                'complete': False,
                'action_name': 'Permanently delete',
...
def anonymize_customer(request, user_id):
        ...
        if not reason:
            ...
            before = {
                ...
                'had_delivery_address': bool(
                    ...
                    or customer.house_number
                    or customer.city
                    or customer.delivery_region
                ),
            }
            if customer.avatar:
            ...
            return _secure(
                render(
                    ...
                    {
                        'complete': True,
                        'action_name': 'Customer anonymisation',
    ...
    return _secure(
        render(
            ...
            {
                'complete': False,
                'action_name': 'Anonymise customer',
