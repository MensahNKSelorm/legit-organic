...
class SubscriptionPlanPriceChangeAdmin(ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            previous_status = (
                SubscriptionPlanPriceChange.objects.filter(pk=obj.pk)
        ...
        if obj.status == 'scheduled' and previous_status != 'scheduled':
            ...
            record_event(
                action='subscription.price_change_scheduled',
                request=request,
                target=obj,
                before={'weekly_price': str(obj.old_price)},
                after={
                    'weekly_price': str(obj.new_price),
    ...
    def retry_failed_notices(self, request, queryset):
        ...
        for change in queryset.filter(status='scheduled'):
            ...
            for notice_id in change.notices.filter(status__in=['pending', 'failed']).values_list(
                'pk', flat=True
            ):
    ...
    def apply_due_changes(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{applied} price change(s) applied. Unnotified customers kept their old price.',
        )
    ...
    def cancel_changes(self, request, queryset):
        for change in queryset.filter(status='scheduled'):
            ...
            change.notices.filter(status__in=['pending', 'sent', 'failed']).update(
                status='cancelled'
            )
            from security.audit import record_event

            record_event(
                action='subscription.price_change_cancelled',
                request=request,
                target=change,
...
class SubscriptionPriceNoticeAdmin(ModelAdmin):
    ...
    def retry_delivery(self, request, queryset):
        for notice_id in queryset.filter(status__in=['pending', 'failed']).values_list(
            'pk', flat=True
        ):
            deliver_price_notice(notice_id)
...
class SubscriptionWeekAdmin(ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            previous_status = (
                SubscriptionWeek.objects.filter(pk=obj.pk).values_list('status', flat=True).first()
            )
        super().save_model(request, obj, form, change)
        if (
...
class WholesaleQuoteItemInline(TabularInline):
    ...
    extra = 0
    fields = ['product', 'quantity', 'requested_unit', 'quoted_unit_price', 'note']
...
class WholesaleQuoteAdmin(ModelAdmin):
    ...
    search_fields = ['business__company_name', 'business__business_email', 'customer_note']
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            previous = (
                WholesaleQuote.objects.filter(pk=obj.pk).values_list('status', flat=True).first()
            )
        super().save_model(request, obj, form, change)
...
class BusinessSupplyAgreementAdminForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if self.instance.pk:
            previous = (
                BusinessSupplyAgreement.objects.filter(pk=self.instance.pk)
                .values_list('status', flat=True)
                .first()
            )
...
class BusinessSupplyAgreementAdmin(ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            previous = (
                BusinessSupplyAgreement.objects.filter(pk=obj.pk)
                .values_list('status', flat=True)
                .first()
            )
        ...
        if previous and previous != obj.status:
            ...
            record_event(
                action='business_supply.status_changed',
                request=request,
                target=obj,
            ...
            if obj.status in {'approved', 'active', 'paused', 'cancelled'}:
                copy = {
                    'approved': (
                        ...
                        'Your supply request is approved.',
                        'Review the agreed quantities, pricing and delivery details in your dashboard.',
                    ),
                    'active': (
                        'Supply schedule active',
                        'Your supply schedule is active.',
                        'Your next delivery and payment window are available in your dashboard.',
                    ),
                    'paused': (
                        'Supply schedule paused',
                        'Your supply schedule is paused.',
                        'No new delivery cycle will be prepared until the schedule is resumed.',
                    ),
                    'cancelled': (
                ...
                try:
                    ...
                    send_b2b_status_email(
                        obj.business,
                        subject=f'{subject} · LO-SUPPLY-{obj.pk}',
                        title=title,
                        message=message,
...
class BusinessSupplyRevisionAdmin(ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            previous = (
                BusinessSupplyRevision.objects.filter(pk=obj.pk)
                .values_list('status', flat=True)
                .first()
            )
        ...
        if previous == 'submitted' and obj.status == 'approved':
            ...
            changes = {key: value for key, value in obj.proposed_changes.items() if key in allowed}
            if changes:
            ...
            record_event(
                action='business_supply.revision_approved',
                request=request,
                target=obj.agreement,
                before={},
                after=changes,
...
class BusinessSupplyCycleAdmin(ModelAdmin):
    list_display = ['delivery_date', 'agreement', 'status', 'subtotal', 'delivery_fee', 'order']
    list_filter = ['status', 'delivery_date']
    search_fields = ['agreement__name', 'agreement__business__company_name', 'payment_reference']
    readonly_fields = [
        'agreement',
        'subtotal',
        'delivery_fee',
