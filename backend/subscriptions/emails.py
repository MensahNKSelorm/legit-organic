...
def send_subscription_created_email(subscription):
    return _send(
        subscription,
        subject='Your weekly delivery plan is ready',
        title='Your week is set',
        paragraphs=[
        ...
        details=[
            ('Plan', subscription.name or 'Weekly basket'),
...
def send_subscription_action_email(subscription, action):
    copy = {
        'paused': (
            'Weekly deliveries paused',
            'Your plan is paused',
            'No new weekly order will be prepared while your plan is paused.',
        ),
        'resumed': (
            'Weekly deliveries resumed',
            'Your plan is active again',
            'We have scheduled your next weekly delivery.',
        ),
        'cancelled': (
            'Weekly deliveries cancelled',
            'Your plan is cancelled',
            'No further weekly orders will be created for this plan.',
        ),
        'skipped': (
            'This week has been skipped',
            'Your next delivery has moved',
            'We skipped the current delivery and scheduled the following week.',
        ),
    }
...
def send_renewal_ready_email(week):
    return _send(
        ...
        paragraphs=[
            'Review the renewal order and pay when you are ready. We will not prepare it until payment is confirmed.'
        ],
        details=[
            ('Delivery date', week.delivery_date.strftime('%d %B %Y')),
...
def send_week_expired_email(week):
    return _send(
        week.subscription,
        subject=f'Delivery window closed for {week.delivery_date:%d %B}',
        title='This delivery has expired',
        paragraphs=[
            'The payment window closed before payment was completed, so this delivery will not be prepared.'
        ],
...
def send_business_cycle_email(cycle, event):
    ...
    return resend.Emails.send(
        payload(
            ...
            paragraphs=[message],
            details=[
                ('Delivery date', cycle.delivery_date.strftime('%d %B %Y')),
...
def send_weekly_payment_link(subscription, week, authorization_url):
    return _send(
        subscription,
        subject=f'Complete payment for {week.delivery_date:%d %B}',
