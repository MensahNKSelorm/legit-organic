...
class SubscriptionAPITests(APITestCase):
    def setUp(self):
        ...
        self.product = Product.objects.create(name='Local rice', price=Decimal('40.00'), unit='2kg')
        self.zone = DeliveryZone.objects.create(
            name='Test Accra',
            delivery_weekday=(timezone.localdate().weekday() + 3) % 7,
    ...
    def test_business_deliveries_cannot_be_created_as_household_subscriptions(self):
        ...
        B2BProfile.objects.create(
            user=self.user,
            company_name='Kitchen Ltd',
            business_type='restaurant',
            contact_person='Ama',
            business_phone='0244000000',
    ...
    def test_skip_creates_next_week(self):
        ...
        self.assertTrue(
            subscription.weeks.filter(delivery_date=old_date, status='skipped').exists()
        )
    ...
    def test_verification_rejects_underpayment(self, mock_verify):
        ...
        response = self.client.post(
            '/api/subscriptions/payment/verify/', {'reference': 'LO-SUB-TEST'}, format='json'
        )
...
class BusinessSupplyAPITests(APITestCase):
    def setUp(self):
        ...
        self.profile = B2BProfile.objects.create(
            user=self.user,
            company_name='Buyer Kitchen Ltd',
            business_type='restaurant',
            contact_person='Ama Buyer',
            business_phone='0244000000',
            business_email=self.user.email,
            business_address='Accra',
            status='approved',
    ...
    def payload(self):
        return {
            ...
            'delivery_zone': self.zone.pk,
            'delivery_address': '1 Trade Road, Accra',
    ...
    def test_business_quote_sends_review_confirmation(self):
        response = self.client.post(
            '/api/subscriptions/business/quotes/',
            {
    ...
    def test_business_supply_rejects_regular_market_product(self):
        ...
        response = self.client.post('/api/subscriptions/business/supply/', payload, format='json')
        self.assertEqual(response.status_code, 400)
    ...
    def test_business_only_sees_its_own_agreements(self):
        ...
        other_profile = B2BProfile.objects.create(
            ...
            business_email=self.other.email,
            business_address='Tema',
            status='approved',
        )
    ...
    def test_cycle_actions_are_scoped_to_business_owner(self):
        ...
        BusinessSupplyCycle.objects.create(
            agreement=agreement,
            delivery_date=agreement.next_delivery_date,
            payment_due_at=timezone.now() + timedelta(days=1),
            subtotal=agreement.subtotal,
            delivery_fee=agreement.delivery_fee,
        )
        response = self.client.post(f'/api/subscriptions/business/supply/{agreement.pk}/skip/')
        self.assertEqual(response.status_code, 200)
...
class SubscriptionPriceChangeTests(APITestCase):
    def setUp(self):
        self.staff = User.objects.create_user(
            email='operations@legitorganic.com',
            password='x',
            is_staff=True,
        )
        ...
        self.subscription = Subscription.objects.create(
            ...
            status='active',
            weekly_subtotal=Decimal('100.00'),
            weekly_delivery_fee=Decimal('10.00'),
        )
        self.change = SubscriptionPlanPriceChange.objects.create(
            plan=self.plan,
            new_price=Decimal('120.00'),
            effective_at=timezone.now() + timedelta(days=15),
            status='scheduled',
            reason='Seasonal sourcing costs have changed.',
